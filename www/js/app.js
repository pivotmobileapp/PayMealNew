/* SENJU THEME FOR KARENDERIA MOBILE APP V2 - 1.3 */
	...
	switch (page_id) {
		...
		case "create_account":
			...
			*/
			...
			$("#stic_app_name").html(krms_config.AppTitle);
			createAccountFields();
		...
		case "home":
			...
			if (isLocation()) {
				if (location_res = getLocationData()) {
					...
					$(".print_location_address").html(t("Please select your location"));
				}
		...
		case "search_category":
			placeholder(".search_category_name", "Search for category");
			setTimeout(function () {
		...
		case "search_form":
			placeholder(".search_restaurant_name", "Search for restaurant, cuisine or food");
		...
		case "search":
			placeholder(".xx", "Search for restaurant, cuisine or food");
		...
		case "cancel_order_form":
			placeholder(".cancel_reason", "Why would you like to cancel the order?");
			order_id = page.data.order_id;
		...
		case "location_city":
			...
			placeholder("#location_city .location_city", "Search for City");
		...
		case "location_area":
			placeholder("#location_area .field_location_area", "Search for District/Area");
		...
		case "location_state":
			...
			placeholder("#location_state .field_location_state", "Search for State");
		...
		case "location_postal_code":
			...
			placeholder("#location_postal_code .field_location_postal_code", "Search for Zip Code");
...
loadHomePage = function () {
	...
	var user_firstname = getStorage("user_firstname");
	...
	if (app_settings.home.mobile2_home_banner == 1) {
		home_banner_count = app_settings.home_banner.stic_banner.length;
		if (home_banner_count > 0) {
			fillHomeBanner(app_settings.home_banner.stic_banner, '.home_banner_wrapper');
		}
			...
			if (data.code == 1) {
				...
				switch (action) {
					...
					case "googleLogin":
						...
						setStorage('user_firstname', data.post.first_name);
...
identifyLocationLoader = function (loading) {
	if (loading) {
		$(".print_location_address").html(t("Loading..."));
...
showNoConnection = function (show) {
	...
	if (d) {
		...
		if (show) {
			ons.createElement('dialog_no_connection.html', {
				append: true
			}).then(function (dialog) {
				dialog.show();
			});
...
runTrackHistory = function () {
	...
	setTimeout(function () {
		track_history_interval = setInterval(function () {
			ExecRunTrackHistory()
		}, track_interval_timeout);
	}, 400);
};
...
showFloatingCategory = function () {
	...
	if (fab_dialog) {
		...
		ons.createElement('floating_category.html', {
				append: true
			})
